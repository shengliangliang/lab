package com.king.algorithm.vehicle;

import java.util.*;
import java.util.stream.Collectors;

public class VehicleScheduling2 {

    public static void main(String[] args) {
        int[][] vehicleLocation = new int[][]{{80,35},{58,43},{17,29},{7,19},{170,409},{157,349},{187,849},{617,49}};
    ...
    private static void findMinDistance(int[][] vehicleLocation, int[][] appraiserLocation) {
        ...
        System.out.println("****第一种方法*****总是距离优先*****总花费："+sum);
        ...
        System.out.println("****第二种方法******中心点距离优先总花费："+sum2);
    ...
    private static void violence(List<List<Double>> cost, Boolean[][] ban2, List<Integer[]> result3) {
        ...
        List<List<Integer>> plan = null;
        for(TreeNode leaf:leafList){
            TreeNode node =  leaf;
            //System.out.print(node.getLevel()+"选择:"+node.index);
            Double sum = cost.get(node.level).get(node.index);

            while (node.parent!=null&&node.parent.getLevel()>-1){
                //System.out.print(node.parent.getLevel()+"选择"+node.parent.getIndex());
            ...
            if(sum<min){
                ...
                chose.add(leaf.getLevel());
                chose.add(leaf.getIndex());
                plan.add(chose);
                //重置为叶子节点，装载选择组合
                node = leaf;
                while (node.parent!=null&&node.parent.getLevel()>-1){
                    ...
                    Integer index = node.parent.index;
                    if(index>-1){
                        List<Integer> chose2 = new ArrayList<>();
                        chose2.add(level);
                        chose2.add(index);
                        plan.add(chose2);
                    }
        ...
        System.out.println("****最少花费是:"+min+",方案组合是："+plan);
    }
